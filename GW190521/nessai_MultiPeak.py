...
outdir = "/data/wiay/undergrads/2385431c/GW190521/MultiPeak_nessai/results"
label = "GW190521MultiPeak"
...
class power_gaussian(bilby.core.prior.Prior):
    ...
    def prob(self, val):
        ...
        return np.nan_to_num( self.beta * val ** self.alpha * np.exp(self.beta * val) * (1 - (self.gamma * self.alpha / self.beta) * np.log(self.maximum/self.minimum)) / (self.gamma * self.maximum ** self.alpha * np.exp(self.beta * self.maximum) - (self.minimum ** self.alpha * np.exp(self.beta * self.minimum))) * in_prior)  * self.is_in_prior_range(val) + (40 * (np.exp(-(self.mu - val) ** 2 / (2 * self.sigma **2)) / (2 * np.pi) ** 0.5 / self.sigma)) + (60 * (np.exp(-(self.mu2 - val) ** 2 / (2 * self.sigma2 **2)) / (2 * np.pi) ** 0.5 / self.sigma2))
...
prior_power_gaussian = power_gaussian(name="name", mu = 35, mu2 = 70, sigma = np.sqrt(10), sigma2 = np.sqrt(70), alpha = 0.31, beta = -0.06, gamma = 3, minimum = 40, maximum = 120)

# this is range of x values (mass) going from just above 0 to 120 solar masses
x, dx = np.linspace(40, 120, 1000, retstep=True)
...
prior_int = Interped(name="mass_1",xx = x,  yy = y_i, dx = dx, mu = 35, mu2 = 70, sigma = np.sqrt(10), sigma2 = np.sqrt(70), alpha = 0.31, beta = -0.06, gamma = 3, minimum = 40, maximum = 120)

'''
plt.figure(figsize=(12,5))
plt.hist(prior_int.sample(100000), bins=100, histtype="step", density=True)
plt.plot(x,  prior_int.prob(np.linspace(prior_int.minimum, prior_int.maximum, 1000)))
plt.xlabel("{}".format(prior_power_gaussian.latex_label)) 
plt.show()
plt.savefig('prob2.jpg')
'''


priors = bilby.gw.prior.BBHPriorDict(filename="MultiPeak.prior")
priors["mass_1"] = Interped(name="mass_1",xx = x,  yy = y_i, dx = dx, mu = 35, mu2 = 70, sigma = np.sqrt(10), sigma2 = np.sqrt(70), alpha = 0.31, beta = -0.06, gamma = 3, minimum = 40, maximum = 120)
priors["mass_2"] = Interped(name="mass_2",xx = x,  yy = y_i, dx = dx, mu = 35, mu2 = 70, sigma = np.sqrt(10), sigma2 = np.sqrt(70), alpha = 0.31, beta = -0.06, gamma = 3, minimum = 40, maximum = 120)
